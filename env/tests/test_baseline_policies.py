...
class TestBaselinePolicies(unittest.TestCase):
    ...
    def test_random_policy_valid_index(self):
        self.player.hand.receive_cards(make_hand([("bastoni", "1"), ("coppe", "3")]))
        policy = RandomPolicy()
    ...
    def test_highest_lead_suit_policy_selects_highest(self):
        self.player.hand.receive_cards(make_hand([
            ("bastoni", "4"),
            ("bastoni", "3"),
            ("coppe", "1")
        ]))
        ...
        self.assertEqual(self.player.hand.cards[index].rank, "3")
        self.assertEqual(self.player.hand.cards[index].suit, "bastoni")
    ...
    def test_heuristic_policy_leading_plays_highest(self):
        ...
        index = policy.get_action_index(self.player, lead_suit=None)
        self.assertEqual(self.player.hand.cards[index].rank, "3")
        self.assertEqual(self.player.hand.cards[index].suit, "denari")
    ...
    def test_heuristic_policy_beats_card_if_possible(self):
        ...
        self.assertEqual(self.player.hand.cards[index].rank, "2")
        self.assertEqual(self.player.hand.cards[index].suit, "spade")
    ...
    def test_heuristic_policy_plays_lowest_if_cannot_win(self):
        ...
        policy = SimpleHeuristicPolicy()
        index = policy.get_action_index(self.player, lead_suit="spade", trick=trick)
        self.assertEqual(self.player.hand.cards[index].rank, "4")  # lower value
        self.assertEqual(self.player.hand.cards[index].suit, "spade")
